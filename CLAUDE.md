...
mission worktree            ──YES COMMIT──▶  claude/<mission>
  ...
  temporary preview, offset >=1                 │ YES INTEGRATE (integrator, ff-only)
                                                ▼
...
There is **no permanent dev server on this box.** A preview is temporary: it
belongs to one mission, it runs while that mission is being reviewed, and it is
stopped when the mission ends. Day to day, the thing to look at is the deployed
site. Nothing reaches it except by being integrated into `working` and released
to `main`.
...
| 5174+ / 3101+ / 5434+ | mission previews, offset 1 and up |

`scripts/dev-preview.sh start` **refuses offset 0.** There is no canonical
preview to protect any more; keeping 5173 free is what makes a stock `pnpm dev`
unambiguous. `PORT_OFFSET` defaults to 1.
...
on its own ports:
    ...
    PORT_OFFSET=1 scripts/dev-preview.sh url       # the URL to hand over
    PORT_OFFSET=1 scripts/dev-preview.sh stop      # when the mission is done

Tell the operator that URL when you hand off, and **stop the preview when the
mission ends** — previews are temporary, and a forgotten one holds ports and a
node process for nothing. Two features previewing at once means two worktrees
and two offsets — never two offsets in one worktree.
...
3. The merge's `post-merge` hook calls `scripts/dev-preview.sh reload` for any
   preview actually running in that worktree, so it shows the result within
   seconds — Vite HMR and
...
4. Log what landed. If the operator wants to see it before release, start a
   preview on `working` from a scratch worktree at a free offset, hand over that
   URL, and stop it when they are done.

Several finished features can sit in `working` at once — that is what it is for.

## Promoting to main (separate, gated)

Manual testing happens on `working`, in a temporary preview. Only after that,
...
- Never run a preview at offset 0 — 5173/3100/5433 are not ours to bind.
- Never develop in the main worktree, and never push `working`.
- Never leave a preview running after its mission ends.
