...
test.describe("board prefix", () => {
  test("renaming the prefix relabels existing card ids", async ({ page }) => {
    ...
    const input = await openPrefixEditor(page, current)
    ...
    await expect(prefixMenuItem(page, target)).toBeVisible()
  })
  ...
  test("rejects a prefix already used by another board", async ({ page }) => {
    ...
    const input = await openPrefixEditor(page, "UB2")
    await input.fill("UB")
    await input.press("Enter")

    await expect(page.getByRole("alert")).toHaveText("UB is taken")
    // Rejected: the modal stays open rather than saving and closing.
  ...
  test("Escape cancels the edit", async ({ page }) => {
    ...
    const input = await openPrefixEditor(page, "UB")
    await input.fill("ZZZZ")
    await input.press("Escape")

    await expect(input).toHaveCount(0)
    await openBoardMenu(page)
    await expect(prefixMenuItem(page, "UB")).toBeVisible()
  })
})
